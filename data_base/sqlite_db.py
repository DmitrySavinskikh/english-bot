...
async def sql_take_set(id_user):
    if await if_not_empty(id_user):
        ...
        ''', [id_user]).fetchone() # fix this moment
...
async def minus_one_repeat(id_user, enword):
    if await if_not_empty(id_user):
        ...
        WITH user_dictionary AS (SELECT * FROM dictionary WHERE id_user = ?)
        SELECT * FROM user_dictionary WHERE repeats = (SELECT MAX(repeats) FROM user_dictionary) ORDER BY RANDOM() LIMIT 1
        ''', [id_user]).fetchone()[5] # fix this moment
        # print(repeats)

        cur.execute('UPDATE dictionary SET repeats = ? WHERE en_word = ? AND id_user = ?', [repeats - 1, enword, id_user]) # fix this moment
        # res = cur.execute('SELECT * FROM dictionary WHERE en_word = ?', [enword]).fetchall() 
        # print(res)

        repeats_after = cur.execute('''SELECT * FROM dictionary WHERE id_user = ? AND en_word = ?''', [id_user, enword]).fetchone()[5] # rename variable
        # print(repeats_after)
        base.commit()
